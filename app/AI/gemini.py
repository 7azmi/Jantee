...
class GeminiAIGenerator:
    ...
    def generate_message(self, user_info, context, language):
        token = self._get_next_token()
        genai.configure(api_key=token)
        prompt_parts = [
            "Jantee is a no-nonsense AI fitness coach who motivates with tough love and a sharp tongue. Expect rigorous pushup training with a side of humor and blunt, motivational banter that cuts to the chase, building your strength and tenacity.",
...
tokens = ["GEMINI_KEY1", "GEMINI_KEY2", "GEMINI_KEY3", "GEMINI_KEY4"]
generator = GeminiAIGenerator(tokens)

import threading

# Assuming you have the GeminiAIGenerator class defined as before

def test(generator, user_info, context, language):
    response = generator.generate_message(user_info, context, language)
    print(response)

# for i in range(4):  # Adjust the range for the number of parallel requests you want to make
#     thread = threading.Thread(target=test, args=(generator, "user full name is Jack, he scored 2 days of zero pushups", "In a brief message, warn user to do pushups, he'll be kicked otherwise from the group", "English"))
#     thread.start()
...
#
# user_info = "Name: Jack"
# context = "User missed 3 days of pushups in a row"
# language = "English"
#
#
